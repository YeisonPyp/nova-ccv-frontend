...
import {
  Program, ActivityWithMetrics, BudgetItem, ScheduleRow, BudgetSummary
} from '../../models/pat.models';
import {
  getProgramStatusLabel, getProgramStatusClass, MONTH_NAMES
...
type TabType = 'info' | 'activities' | 'budget' | 'schedule' | 'goals' | 'execution';
...
@Component({
  ...
  imports: [
    ...
    StatusBadgeComponent, PatProgressBarComponent, StrategicGoalsPanelComponent,
  ],
...
export class ProgramDetailComponent implements OnInit, OnDestroy {
  ...
  budgetByRubric = computed<Array<{ rubric: string; planned: number; executed: number; pct: number }>>(() => {
  ...
  submitExecution(): void {
    ...
    .subscribe({
      ...
      error: err => {
        this.submitError.set(err.error?.message ?? 'Error al registrar ejecución');
        this.submitting.set(false);
      },
    });
  }
