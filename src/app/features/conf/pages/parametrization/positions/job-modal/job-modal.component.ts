...
export class JobModalComponent implements OnChanges {
    ...
    if (changes["jobData"] && this.jobData && this.isOpen) {
      ...
      if (this.jobData.competencies?.length) {
        this.selectedCompetencies.set(
          this.jobData.competencies.map((i) => ({ id: i.id, title: i.name })),
        );
      }
    }
  }
  ...
  onSearchCompetencie(q: string) {
    this.competencieService
      .getCompetencies({ name: q, size: 10 })
      .subscribe((r) => {
        if (r.success && r.data) {
          this.competencies.set(
            r.data.content.map((e) => ({ id: e.id, title: e.name })),
          );
        }
      });
  }
  ...
  onSelectCompetencie(o: SearchSelectOption) {
    const record = this.selectedCompetencies().reduce(
      (prev, curr) => {
        prev[curr.id] = curr;
        return prev;
      },
  ...
  onRemoveCompetencie(o: SearchSelectOption) {
    this.selectedCompetencies.set(
      this.selectedCompetencies().filter((i) => i.id !== o.id),
    );
  }
